...
int	main(void)
{
	...
	t_expected_token	expected[] = {{T_WORD, "ls"}, {T_WORD, "-la"}, {T_WORD,
			"/home"}};
	t_expected_token	expected[] = {{T_WORD, "echo"}, {T_WORD, "hello"},
			{T_WORD, "world"}};
	t_expected_token	expected[] = {{T_WORD, "cat"}, {T_REDIRECT_IN, "<"},
			{T_WORD, "input.txt"}};
	t_expected_token	expected[] = {{T_WORD, "echo"}, {T_WORD, "hello"},
			{T_REDIRECT_OUT, ">"}, {T_WORD, "output.txt"}};
	t_expected_token	expected[] = {{T_WORD, "echo"}, {T_WORD, "hello"},
			{T_APPEND, ">>"}, {T_WORD, "output.txt"}};
	t_expected_token	expected[] = {{T_WORD, "cat"}, {T_HEREDOC, "<<"},
			{T_WORD, "EOF"}};
	t_expected_token	expected[] = {{T_WORD, "cat"}, {T_REDIRECT_IN, "<"},
			{T_WORD, "in.txt"}, {T_REDIRECT_OUT, ">"}, {T_WORD, "out.txt"}};
	t_expected_token	expected[] = {{T_WORD, "cat"}, {T_REDIRECT_IN, "<"},
			{T_WORD, "input.txt"}, {T_REDIRECT_OUT, ">"}, {T_WORD,
			"output.txt"}};
	t_expected_token	expected[] = {{T_WORD, "ls"}, {T_PIPE, "|"}, {T_WORD,
			"grep"}, {T_WORD, "txt"}};
	t_expected_token	expected[] = {{T_WORD, "cat"}, {T_WORD, "file.txt"},
			{T_PIPE, "|"}, {T_WORD, "grep"}, {T_WORD, "error"}, {T_PIPE, "|"},
			{T_WORD, "wc"}, {T_WORD, "-l"}};
	t_expected_token	expected[] = {{T_WORD, "cat"}, {T_REDIRECT_IN, "<"},
			{T_WORD, "input.txt"}, {T_PIPE, "|"}, {T_WORD, "grep"}, {T_WORD,
			"hello"}, {T_REDIRECT_OUT, ">"}, {T_WORD, "output.txt"}};
	...
	t_expected_token	expected[] = {{T_WORD, "echo"}, {T_WORD, "hello"}};
	t_expected_token	expected[] = {{T_WORD, "cat"}, {T_HEREDOC, "<<"},
			{T_REDIRECT_IN, "<"}, {T_WORD, "file.txt"}};
	t_expected_token	expected[] = {{T_WORD, "echo"}, {T_WORD, "hello"},
			{T_INVALID, "||"}, {T_WORD, "echo"}, {T_WORD, "world"}};
	t_expected_token	expected[] = {{T_WORD, "cat"}, {T_REDIRECT_IN, "<"},
			{T_WORD, "input.txt"}, {T_PIPE, "|"}, {T_WORD, "grep"}, {T_WORD,
			"hello world"}, {T_PIPE, "|"}, {T_WORD, "wc"}, {T_WORD, "-l"},
			{T_REDIRECT_OUT, ">"}, {T_WORD, "output.txt"}};
	t_expected_token	expected[] = {{T_WORD, "echo"}, {T_WORD,
			"User: testuser"}, {T_WORD, "literal $HOME"}};
	t_expected_token	expected[] = {{T_WORD, "cat"}, {T_WORD,
