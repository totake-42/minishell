...
/*   Updated: 2025/12/18 23:00:24 by totake           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	find_env_unset(char *key, t_data *data)
{
	int		i;
	size_t	len;

	if (key == NULL || data->envp == NULL || ft_strchr(key, '=') != NULL)
		return (-1);
	len = ft_strlen(key);
	i = 0;
	while (data->envp[i])
...
int	builtin_unset(t_cmd *cmd, t_data *data)
{
	...
	{
		env_i = find_env_unset(cmd->argv[i], data);
		if (env_i != -1)
